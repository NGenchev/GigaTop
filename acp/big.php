...
<body>
		...
		<HR />
		<hgroup>
				<h3>336х280 Банери</h3>
			</hgroup>
				<HR /><BR />
			<article>
			<h3 class='page-title'>Добавени 336х280 банери:</h3>
			<p class='num'>В сайта има общо <?php echo $num; ?> 336х280 банера</p>
			<?php 
			...
			If ($nums == 0){
				...
				while ($row = mysqli_fetch_assoc($get88x31)){
					...
					echo "<div class='big'><strong>#$id</strong> 
					Link: <a href='$link'>$link</a><BR />
					<img src='$img' alt=''/>
					<form method='POST' action='big.php?id=$id'>
					...
					if(isset($_POST['delete'])){
						$id = $_GET['id'];
						$delete = "DELETE FROM big WHERE big_id = '$id'";
			...
			<BR /><BR />
			<article>
			<h3 class='page-title'>Добави 336х280 банер:</h3>
			<div class="container">
			  <div id="login-form2">
			...
			<?php
				if(isset($_POST['add'])):
					...
					$sql = "insert into big(`big_link`, `big_img`) values ('$link', '$img')";
					$sql = mysqli_query($conn, $sql) or die(mysqli_error($conn));
					if(!$sql){
