<?php	
	...
	include '../conf.php';
	db_con();
	$sql= mysqli_query($conn, "SELECT * FROM small ORDER by small_id DESC") or die(mysqli_error($conn));
...
<body>
		...
		<hgroup>
				...
				<HR /><BR />
			<article>
			<h3 class='page-title'>Добавени 88х31 банери:</h3>
			<p class='num'>В сайта има общо <?php echo $num; ?> 88х31 банера</p>
			<?php 
			...
			If ($nums == 0){
				...
				while ($row = mysqli_fetch_assoc($get88x31)){
					...
					echo "<div class='small'><strong>#$id</strong> 
					Link: <a href='$link'>$link</a><BR />
					<img src='$img' alt=''/>
					<form method='POST' action='small.php?id=$id'>
					...
					if(isset($_POST['delete'])){
						$id = $_GET['id'];
						$delete = "DELETE FROM small WHERE small_id = '$id'";
			...
			<BR /><BR />
			<article>
			<h3 class='page-title'>Добави 88х31 банер:</h3>
			<div class="container">
			  <div id="login-form2">
			...
			<?php
				if(isset($_POST['add'])):
					...
					$sql = "insert into small(`small_link`, `small_img`) values ('$link', '$img')";
					$sql = mysqli_query($conn, $sql) or die(mysqli_error($conn));
					if(!$sql){
